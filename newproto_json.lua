...
function FastBytesToString(bytes)
  ...
  --return "\"[" .. string.gsub(table.concat(s), "\"", "") .. "]\""
  --return "\"[" .. table.concat(s) .. "]\""
end
...
function dns.dissector(tvbuf,pktinfo,root)
   ...
   elseif msg_type == "SYNC" then
    ...
    elseif msg_type == "TREE_INTEREST_QUERY" or msg_type == "TREE_INTEREST_QUERY_ACK" or msg_type == "JOIN_TREE" 
        or msg_type == "JOIN_TREE_ACK" or msg_type == "SET_TREE" or msg_type == "SET_TREE_ACK" or msg_type == "REMOVE_TREE"
        or msg_type == "REMOVE_TREE_ACK" or msg_type == "CONFIRM" or msg_type == "CONFIRM_ACK" then
      queries_tree:add(pf_tree_source, tostring(tab["DATA"].SOURCE))
      queries_tree:add(pf_tree_group, tostring(tab["DATA"].GROUP))
    elseif msg_type == "ACTIVE_TREES" or msg_type == "ACTIVE_TREES_ACK" then
      for i, v in ipairs(tab["DATA"]) do
          local tree_info = queries_tree:add("Tree"..i)
