...
class PacketProtocolAssert:
    ...
    def parse_bytes(cls, data: bytes):
        ...
        pim_payload = cls(multicast_group_address, source_address, metric_preference, metric)
        return pim_payload


class PacketProtocolAssertReliable(PacketProtocolAssert):
    PIM_TYPE = "ASSERT_RELIABLE"

    def __init__(self, multicast_group_address: str or bytes, source_address: str or bytes,
                 metric_preference: int or float, metric: int or float):
        super().__init__(multicast_group_address, source_address, metric_preference, metric)


class PacketProtocolAssertReliableAck(PacketProtocolAssert):
    PIM_TYPE = "ASSERT_RELIABLE_ACK"
