...
class InterfaceProtocol(Interface):
    ...
    def send_reliably(self, data: bytes, id_reliable: int, group_ip: str=MCAST_GRP):
        # TODO
        import ipaddress
        from ReliableMsgTransmission import ReliableTransmission
        if ipaddress.IPv4Address(group_ip).is_multicast:
            rmsg = ReliableTransmission(self, data, neighbors=self.neighbors.keys())
        else:
            rmsg = ReliableTransmission(self, data, neighbors=group_ip)
    ...
    def receive_join(self, packet):
        pkt_join = packet.payload.payload  # type: PacketProtocolJoin

        source_group = (pkt_join.source, pkt_join.group)
        try:
            Main.kernel.get_routing_entry(source_group).recv_join_msg(self.vif_index, packet)
        except:
            traceback.print_exc()
    ...
    def receive_remove_tree_ack(self, packet):
        ip = packet.ip_header.ip_src
        id_reliable = packet.payload.id_reliable
        self.reliable_transmission_buffer[id_reliable].receive_ack(ip)
        return

    def receive_set_tree(self, packet):
        return

    def receive_set_tree_ack(self, packet):
        ip = packet.ip_header.ip_src
        id_reliable = packet.payload.id_reliable
        self.reliable_transmission_buffer[id_reliable].receive_ack(ip)
        return
