...
proctype InterfaceSend(byte node_id; byte interface_id) {
  ...
  :: INTERFACE_TYPE(node_id, interface_id) == root && (CURRENT_TREE_STATE(node_id) != tree_state) ->
    atomic {
    tree_state = CURRENT_TREE_STATE(node_id);
    // talvez interesse
    //skip;
  // interface do tipo non-root
    }
  :: INTERFACE_TYPE(node_id, interface_id) == non_root  && CURRENT_TREE_STATE(node_id) != tree_state ->
    atomic {
    printf("Interface ID %d entrou\n", interface_id);
    if
    :: CURRENT_TREE_STATE(node_id) == active_tree ->
      printf("1 > Interface ID %d entrou\n", interface_id);
      tree_state = active_tree;
      //send i am upstream
    ...
    :: tree_state == active_tree && (CURRENT_TREE_STATE(node_id) == inactive_tree || CURRENT_TREE_STATE(node_id) == unknown_tree) ->
